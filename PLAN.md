...
- [x] **(9/5 전체 평가 — `docs/시스템-평가-2026-09-05.md`) P0 4건 완료(9/5 밤, 4커밋 push)**: ①KR 재무 공시캘린더 순 조회+펀더 as-of(라이브 002790 반기 수신) ②시세 신선도 봉날짜·시장세션 기준+캐시 수명 단일화 ③하트비트=전송 성공 후 기록·워치독 판정 전환·exit 2·review 선실행·WakeToRun 재등록 ④알림·시드 결측 게이트+fundamental 미가용 None. **다음 CI 스캔(9/8 07:00 KST)에서 KR 전 종목 재무 전량 재조회 → KR 랭킹 이동 예상**
- [x] **P1 완료(9/6, 4커밋)**: 계좌 히트 한 줄+⛔ 신규 차단(첫 실측 16.4%/6%) / 페이퍼 손절 자동 집행(이탈 익일 종가, close_position 전 트랜치) / 결정로그 트레일러+decisions.jsonl+`CONTROL.md` 대조군 26건 소급(결정일 종가)+TRACKING「판정별 사후 성과」 / `planned_events.json` 36건 복원+`compliance.py`(정시율 52%·집행률 88%)+review 일일 기한 경과·임박 줄
- [x] **선제 알림(9/6 사용자 요청 '때 되면 알려줘')**: 워치 tranche2/rank_check + brief.py 자동 브리핑(9/12 예약). 다음 후보: 10/10 분기 리뷰 자료팩 kind 추가
- [x] **stock-investing 비공개 GitHub 이관(9/6)**: 저널 git 이력 + 개인 JSON 정본을 stock-investing/data/로(스크립트 폴백 유지), daily.ps1 자동 커밋·push. 다음 후보: 비공개 저장소 Actions로 brief 실행(PC 무관 알림)
- [x] **인시던트 대응(9/8)**: 휴장일 신선도 오탐·감시 경로 라이브 폴백·고아 프로세스 3건 수정(`docs/인시던트-2026-09-08.md`). 잔여 관찰: 08:10 정규 실행 미발생(PC 꺼짐) — 외부 dead-man(HEALTHCHECK_URL) 미설정 상태
- [ ] **P1 잔여(9월 중)**: recommend 대조군 자동 코호트(게이트 통과·미채택 전량 시드) / 랭킹 체크 스냅샷 신선도 게이트 / 페이퍼·실계좌 동일 티커 분리 추적은 9/6 반영
- [ ] **웨이크 타이머 신뢰 불가 실증(9/12)**: DC(배터리) RTCWAKE 활성화(9/9)에도 PC가 자정~11:33 절전 중 08:10·09:00 트리거 둘 다 못 깨움(Modern Standby 특성). brief.py는 세션에서 수동 실행으로 대체·성공. 근본 해결은 (a) healthchecks.io 외부 감시 (b) 실행시각을 사용자 기상 시간대로 이동 중 택1 — 사용자 결정 대기.
- [ ] **P2 밸류·게이트 결함 4건(9/18 사이클 #8 적출)**: ①PODD PER 49.16 = 오류(실제 TTM 26.7·선행 20.5) — 밸류 사이드카 소스 점검 ②**5년고점 -50% 게이트가 상품주에 오작동** — VET의 분모가 2022 유럽 가스위기 일회성 피크라 '저점 대비 +77% 회복한 종목'을 폭락주로 포착. 에너지·원자재에 중앙값 낙폭 또는 사이클 위치 필터 검토 ③**GAAP EPS 밸류팩터가 일회성 항목에 2배 오판** — CAL의 관세환급이 COGS 경유로 GM 800bp·EPS 2배 부풀림. 조정 EPS 병행 필요 ④헤지북 MTM이 F-score·Altman 왜곡 소지(VET GAAP 순손실 C$653.6M) + 리스부채·자산제거의무가 레버리지 지표에 포함되는지 확인
- [ ] **P2 장중 부분봉 가드(9/8 실측)**: 9/8 수동 스캔이 미 동부 09:36(개장 6분 후)에 돌아 US 최신 봉이 **장중 부분봉**으로 스냅샷에 들어갔다(정규 cron 22:00 UTC는 마감 2.5h 후라 안전 — 수동·재실행이 위험). 가드: 시장 세션 중이면 당일 행 drop 또는 abort.
- [ ] **P2 데이터 위생(9/6 발견)**: yfinance 분기 재무 행에 반기 누적이 섞임(AMN 2026-03-31 매출 $1,378M = 2분기치 → 사이드카 YoY +99.9%, 랭킹 자격 오염). 가드: 인접 분기 중앙값의 1.7배 초과 행 무효화(NaN) + health에 이상치 수 표기. MNSO 2026-06-30 행은 손익 NaN(부분 파싱).
- [ ] **P2 (10/10 전)**: 점수 드리프트 3건(Altman EBIT 연율화·부채 정의 통일·KR 4Q 캐시) 섀도 비교 후 전환 / DART 위험신호 3상태 / KR 장중 부분봉 가드 / stock-investing git init·정합성 검사 / 위생 묶음
- [ ] (선택) healthchecks.io 체크 생성 → `.env`의 `HEALTHCHECK_URL` — PC가 꺼진 날을 바깥에서 잡는 유일한 경로
...
- [ ] **(10/10 분기 리뷰 안건 — 사용자 요청 등재 7/20)** ①랭크컷·ATR 문턱 재조정(이연분) ②**진입 타이밍 신호 재검증**(전문 에이전트 + 2026-07 폭락장 out-of-sample 데이터 포함 — 기존 음성결과(off_lows t−5.5 등, timing-signals.md)의 재현 여부 확인. 단 재검증도 PIT 백테스트 통과 전엔 운영 반영 금지) ③페이퍼 12주 규칙 준수율·베토 실효성 판독 ④**(9/6 실험 추가)** KR 패널 위생 정정 반영(상폐-only 17날짜 제외한 클린 재계산으로 문서 갱신) ⑤**바스켓 폭**: 상위 50~80 바스켓 엣지 t 2.4~4.3 vs 상위 5~30 노이즈 → 페이퍼 '상위 25% 바스켓' arm 신설 여부 ⑥회전(60/120d) KR-only 시사 → forward 3창 병기 ⑦off_lows 잔차 유의(KR t −3.9) 재논의(행동규칙 유지 기본) · 기각 확정: 보유 중 레짐 청산, drop5(ATR 흡수), 폭락 유형 게이트 — `docs/experiment-*-2026-09-06.md`
