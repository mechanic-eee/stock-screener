...
- [ ] **(다음) 주간 사이클 반복 + 감사 P1** — 매 주말 recommend.py(⭐5부터) → 체크리스트 → 페이퍼 기록. **P1 최우선: track (티커,날짜) 복합키+가중평단 — 8/10 2차 트랜치 전 필수.** 그 외 P1: 레짐 fail-closed 완성·DART/EDGAR 딥링크·알림 보유 1줄·이벤트 리마인더·TRACKING 벤치마크 병기·보유 랭킹강등 체크·섹터 매핑. 60d 중간성적으로 규칙 수정 금지.
